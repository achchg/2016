...
```{r}
tab %>%
  filter(end_date < "2008-01-01") %>%
  mutate(diff = Obama - McCain) %>%
  summarize(theta = mean(diff), sd = sd(diff))
```
...
```{r}
tabEst = tab %>%
  filter(start_date > "2008-01-01" & week <= -30) %>%
  mutate(diff = Obama - McCain)%>%
  group_by(week) %>%
  filter(n()>5) %>%
  ungroup %>%
...
```{r}
tabEst$theta_hat + qnorm(0.975)*c(-1,1)*tabEst$sigma_hat/sqrt(tabEst$tot_poll)
```
...
```{r}
pnorm((0.012390655-tabEst$theta_hat)/tabEst$sigma_hat)-pnorm((-0.009339808-tabEst$theta_hat)/tabEst$sigma_hat)
```
...
tab %>%
  mutate(diff = Obama - McCain) %>%
  filter(week > -5 & N==1000) %>%
  summarize(theta = mean(diff), sigma = 2*sqrt(theta*(1-theta)/1000))
```
