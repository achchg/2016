...
a = sapply(1:s, function(i){
  poll = sample(x, n, replace = T)
  p_hat <- mean(poll=="D")
  e = p_hat-p
  })
hist(a)
...
poll = sample(x, 625)
p_hat <- mean(poll)
ci <- p_hat + c(-1,1)*qnorm(0.995)*sqrt(p_hat*(1-p_hat))/sqrt(N)
p 
ci[1] <= p & ci[2] >= p
```
...
N = 400
x = -1/19
sd = 2*sqrt(10/19*9/19)
simulation = 10000
error = replicate(simulation, {
...
```{r}
1 - pnorm((0 - x)/(sd/sqrt(N)))
simulation = 10000
p0 = 10/19
N = 400
earn = replicate(simulation, {
  X = sample( c(-1,1), N, replace = TRUE, prob=c(10/19, 9/19)) 
  sum(X)})
mean(earn>0)
```
...
```{r}
# 1-((0-x)/(sd/sqrt(N))) = 0.001
n = (qnorm(0.999)*(sd/(-x)))^2
round(n)
N = n
...
```{r}
1-pnorm(-1/(sd(y)/sqrt(n)))*2

pnorm(sqrt(n)/sd(y))-pnorm(-sqrt(n)/sd(y))
```
...
```{r}
se = mean(p)*(1-mean(p))/sqrt(N)
ci = c(mean(p)-qnorm(0.975)*se, mean(p)+qnorm(0.975)*se)
```
...
a = replicate(10000,{ 
  p = sample(c(1,0), N, replace = T,
         prob = c(prob_heads,(1-prob_heads)))
  se = sqrt(mean(p)*(1-mean(p))/N)
  ci = mean(p)+c(-1,1)*qnorm(0.975)*se
