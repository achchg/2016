...
fit = dat2 %>% 
  do(augment(lm(y ~ x, data = .), data = .))
fit %>%
  filter(nameFirst == "Miguel")
...
Teams %>%
  filter(yearID >= 1961 & BB+AB > 500 & !is.na(HR) & !is.na(BB)) %>% 
  summarize(cor_R_BB = cor(R,BB), cor_R_HR = cor(R,HR), cor_BB_HR = cor(BB,HR))
...
# Teams %>%
#   filter(yearID >= 1961 & BB+AB > 500 & !is.na(HR) & !is.na(BB)) %>% 
#   mutate(group = cut(HR, quantile(HR, prob= seq(0,1,.1)), include.lowest = T))%>%
#   ggplot(aes(R,BB))+facet_wrap(~group)+geom_point()+geom_smooth(method = "lm", se = T, col = "blue")
