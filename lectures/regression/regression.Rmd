...
$$
\mbox{Var}(Y \mid X=x ) = ({1-\rho^2}) \sigma_y^2
$$
...
1. We use $\gamma$ instead of $\beta$ and $\delta$ instead of $\epsilon$ because this is a different model. We can't solve the first equation to get the second because $\varepsilon$ is a random variable. 
...
# or
res <- summary(fit)
res$coef[2,1] + c(-1,1)*qt(0.975, 28)*res$coef[2,2]
```
